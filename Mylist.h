 ...
 * @brief Template definitions for Newlists.  Specifically, include this file to
 * create Newlist classes in a manner similar to STD lists
 * @version 0.1
 * @date 2019-10-07
 *
 ...
 public:
  ...
  class const_iterator {
   ...
   protected:
    ...
    Node *mCurrent;
    const Mylist *mMyList;  // needed to test for certain errors

    // protected constructor for use only by derived iterator and friends
    const_iterator(Node *p, const Mylist<Object> &lst)
        : mCurrent(p), mMyList(&lst) {}

   public:
    const_iterator() : mCurrent(NULL), mMyList(NULL) {}

    const Object &operator*() const {
      if (!mCurrent) throw NullIteratorException();
      return mCurrent->data;
    }

    const_iterator &operator++() {
      if (mCurrent->next != NULL) mCurrent = mCurrent->next;
      return *this;
    }
    ...
    const_iterator &operator--(int) {
      ...
      old = *this;
      if (mCurrent->prev != NULL) mCurrent = mCurrent->prev;
      return old;
    }
  ...
  class iterator : public Mylist<Object>::const_iterator {
    friend class Mylist;

   protected:
    // chain to base class
    iterator(Node *p, const Mylist<Object> &lst) : const_iterator(p, lst) {}

   public:
    iterator() {}
    const Object &operator*() const { return const_iterator::operator*(); }
    Object &operator*() {
      if (!this->mCurrent) throw NullIteratorException();
      return this->mCurrent->data;
    }
    iterator &operator++() {
      if (this->mCurrent->next != NULL) this->mCurrent = this->mCurrent->next;
      return *this;
    }
    ...
    iterator &operator--() {
      if (this->mCurrent->prev != NULL) this->mCurrent = this->mCurrent->prev;
      return *this;
    }
    iterator &operator--(int) {
      static iterator old;
  ...
  const_iterator end() const { return const_iterator(mTail, *this); }
  ...
  iterator insert(iterator iter, const Object &x) {
    if (iter.mMyList != this) throw IteratorMismatchException();

    Node *p = iter.mCurrent;
    if (p == NULL) throw NullIteratorException();
    if (p->prev == NULL) throw NullIteratorException();
  ...
  iterator erase(iterator iter) {
    ...
    Node *p = iter.mCurrent;
    if (p == NULL) throw NullIteratorException();
    if (p->prev == NULL || p->next == NULL) throw NullIteratorException();
  ...
  iterator erase(iterator start, iterator stop) {
    ...
    for (iterator iter = start; iter != stop;)
      iter =
          erase(iter);  // if this throws, then mCurrent erase() will throw, too
...
// public interface
template <class Object>
void Mylist<Object>::clear() {
  while (mSize > 0) pop_front();
}

template <class Object>
void Mylist<Object>::pop_front() {
  Node *p;
...
template <class Object>
void Mylist<Object>::push_front(const Object &x) {
...
template <class Object>
void Mylist<Object>::push_back(const Object &x) {
...
template <class Object>
const Mylist<Object> &Mylist<Object>::operator=(const Mylist &rhs) {
  const_iterator iter;
  if (&rhs == this) return *this;
  clear();
  for (iter = rhs.begin(); iter != rhs.end(); ++iter) this->push_back(*iter);
  return *this;
}

// definition of nested Mylist<Object>::Node class ----------------------
template <class Object>
class Mylist<Object>::Node {
 public:
  Node *prev, *next;
  Object data;
